...
export async function listVercelDeployments(
  token: string,
  params: { projectId: string; teamId?: string; limit?: number },
...
export async function redeployVercelDeployment(
  token: string,
  deploymentId: string,
  params?: { target?: 'production' | 'preview'; teamId?: string },
): Promise<VercelDeployment> {
  const url = new URL(`${VERCEL_API}/v13/deployments/${deploymentId}/redeploy`)
  if (params?.teamId) url.searchParams.set('teamId', params.teamId)
