...
export async function loader({ request }: { request: Request }) {
  ...
  if (projectIds.length === 0) {
    log.debug(
      { orgId: org.id },
      "team tasks list: no projects in org, returning empty",
    );
  ...
  const rowsWithContext = await db
    ...
    .where(
      and(inArray(tasks.projectId, projectIds), eq(tasks.createdBy, user.id)),
    )
  ...
  return Response.json(
    {
      task: {
        ...
        vercelProjectId,
        vercelProjectName:
          vercelProjectName ?? existingProject?.vercelProjectName ?? null,
