...
export async function loader({
  ...
  try {
    ...
    if (!code || !state || state !== storedState) {
      log.warn(
        { provider, state, storedState, hasCode: !!code },
        "auth callback: invalid state or missing code",
      );
      return Response.json(
        {
          error: "Invalid OAuth callback",
          detail: { hasCode: !!code, stateMatch: state === storedState },
        },
        { status: 400 },
      );
    }
    ...
    if (connectOrgId && provider === "github") {
      const returnTo =
        parseCookie(cookieHeader, "connect-return-to") ?? "/onboarding";
      cleanupHeaders.append(
        "Set-Cookie",
        deleteCookieHeader("github-connect-org"),
      );
      cleanupHeaders.append(
        "Set-Cookie",
        deleteCookieHeader("connect-return-to"),
      );
      ...
      if (!sessionToken) {
        ...
        if (!result) {
          ...
          const [membership] = await db
            ...
            .where(
              and(
                eq(orgMembers.userId, result.user.id),
                eq(orgMembers.organizationId, connectOrgId),
              ),
            );

          if (!membership) {
            log.warn(
              { userId: result.user.id, orgId: connectOrgId },
              "github connect: user not a member of org",
            );
          } else {
    ...
    return redirect("/dashboard", { headers: cleanupHeaders });
  } catch (err: any) {
    log.error(
      { error: err?.message, stack: err?.stack },
      "auth callback: unhandled error",
    );
    return Response.json(
      {
