...
export async function action({
  ...
  if (!app) {
    log.warn({ userId: user.id, environmentId }, 'merge: app not found or not in org')
  ...
  if (!task) {
    log.warn({ userId: user.id, environmentId, taskId }, 'merge: task not found')
  ...
  try {
    ...
    log.info(
      { userId: user.id, environmentId, taskId, pr: `${parsed.owner}/${parsed.repo}#${parsed.number}` },
