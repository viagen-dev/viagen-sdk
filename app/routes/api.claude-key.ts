...
function pathForScope(scope: string, orgId: string, userId: string): string | null {
...
export async function loader({ request }: { request: Request }) {
  ...
  if (scope !== 'org' && scope !== 'user') {
    return Response.json({ error: 'scope must be "org" or "user"' }, { status: 400 })
  }
...
export async function action({ request }: { request: Request }) {
  ...
  if (request.method === 'PUT') {
    ...
    if (scope === 'org' && role !== 'admin') {
      return Response.json({ error: 'Admin role required to set org key' }, { status: 403 })
    }
  ...
  if (request.method === 'DELETE') {
    ...
    if (scope === 'org' && role !== 'admin') {
      return Response.json({ error: 'Admin role required to remove org key' }, { status: 403 })
    }
