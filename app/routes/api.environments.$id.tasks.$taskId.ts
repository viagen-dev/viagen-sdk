...
import { environments, tasks, users, orgMembers, taskAttachments } from "~/lib/db/schema";
    ...
    try {
      ...
      if (githubToken && parsed) {
        ...
        if (merged) {
          ...
          log.info({ environmentId, taskId }, "task detail: PR merged, task auto-completed");
        }
      }
    } catch (err) {
      log.warn(
        { environmentId, taskId, error: err instanceof Error ? err.message : "unknown" },
...
export async function action({
  ...
  if (body.projectId !== undefined) {
    const [newApp] = await db
      ...
      .where(
        and(
          eq(environments.id, body.projectId),
          eq(environments.organizationId, org.id),
        ),
      );
    if (!newApp) {
      log.warn(
        { userId: user.id, newAppId: body.projectId },
        "task update: target app not found or not in org",
      );
      return Response.json(
  ...
  if (body.projectId !== undefined) updates.environmentId = body.projectId;
  ...
  log.info(
    {
      ...
      appChanged: body.projectId !== undefined ? body.projectId : undefined,
    },
    "task updated",
  );
