...
export async function loader({
  ...
  const config = (app.vercelEnvSync as Record<string, boolean>) ?? {};

  log.info(
    { environmentId: id, orgId: org.id, keyCount: Object.keys(flat).length },
...
export async function action({
  ...
  if (request.method === "POST") {
    ...
    if (toSync.length > 0) {
      ...
      await upsertVercelEnvVars(vercelToken, app.vercelProjectId, toSync);
    }

    log.info(
      { environmentId: id, synced, skipped, denylisted },
  ...
  if (request.method === "PATCH") {
    ...
    const existing = (app.vercelEnvSync as Record<string, boolean>) ?? {};
