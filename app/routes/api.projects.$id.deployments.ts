...
export async function loader({
  ...
  if (!project) {
    log.warn({ projectId, orgId: org.id }, "deployments: project not found");
    return Response.json({ error: "Project not found" }, { status: 404 });
  }

  if (!project.vercelProjectId) {
    log.debug({ projectId }, "deployments: no Vercel project linked");
  ...
  if (!token) {
    log.debug({ projectId }, "deployments: no VERCEL_TOKEN available");
    return Response.json({ deployments: [] });
  }
...
export async function action({
  ...
  try {
    ...
    if (deployments.length > 0) {
      ...
      const [ghOrg, repo] = project.githubRepo.split("/");
