...
export default function MyTasksPage({
  ...
  const handleTaskClick = useCallback(
    (task: FeedTask) => {
      ...
      navigate(`/projects/${task.projectId}/tasks/${task.id}?from=tasks`);
    },
    [navigate],
  );
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((t) => {
      if (hasProjectFilter && !filters.projectIds.has(t.projectId))
        return false;
  ...
  const handleCreateTask = useCallback(async () => {
    const firstProject = loaderData.projects[0];
    if (!firstProject) {
    ...
    try {
      const res = await fetch(`/api/projects/${firstProject.id}/tasks`, {
        method: "POST",
        credentials: "include",
      ...
      task.projectName = firstProject.name;
      task.githubRepo = firstProject.githubRepo;
      task.vercelProjectId = firstProject.vercelProjectId;
      task.vercelProjectName = firstProject.vercelProjectName;
