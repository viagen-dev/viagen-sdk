...
export async function action({
  ...
  if (!isAdminRole(role)) {
    log.warn(
      { userId: user.id, orgId: org.id, environmentId: params.id },
  ...
  if (!app) {
    log.warn(
      { userId: user.id, environmentId: id },
      "sandbox launch: app not found",
    );
    return Response.json({ error: "App not found" }, { status: 404 });
  }
  ...
  if (request.method === "DELETE") {
    ...
    if (workspace.taskId) {
      ...
      if (linkedTask && linkedTask.status === "running") {
        ...
        log.info(
          { taskId: linkedTask.id, environmentId: id },
  ...
  if (!app.githubRepo) {
    log.warn({ environmentId: id }, "sandbox launch: no github repo linked");
    return Response.json(
      { error: "App must have a GitHub repo to launch a sandbox" },
      { status: 400 },
    );
  }
  ...
  log.info(
    {
      ...
      hasVercelProjectId: !!app.vercelProjectId,
      hasVercelOrgId: !!app.vercelOrgId,
  ...
  const remoteUrl = `https://github.com/${app.githubRepo}.git`;
    ...
    try {
      ...
      if (!repoCheck.ok) {
        ...
        if (repoCheck.status === 404) {
          return Response.json(
            {
              error: `Repository ${app.githubRepo} not found or token lacks access. Check that the repo exists and your GitHub token has the repo scope.`,
            },
            { status: 400 },
          );
        }
      ...
      log.info(
        { environmentId: id, repo: app.githubRepo },
  ...
  try {
    ...
    const [workspace] = await db
      ...
      .values({
        ...
        vercelOrgId: app.vercelOrgId ?? null,
...
- viagen_list_tasks — List tasks in this app. Use status to filter (ready, running, validating, completed, timed_out).
...
- viagen_list_tasks — List tasks in this app. Use status to filter (ready, running, validating, completed, timed_out). Call this first to understand what's been done and what's pending.
- viagen_get_task — Get full details of a task by ID, including its prompt, status, branch, and PR URL.
