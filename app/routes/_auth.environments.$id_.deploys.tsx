...
export default function AppDeploys({
  loaderData,
}: {
  loaderData: { app: AppInfo; role: string };
}) {
  const { app, role } = loaderData;
  ...
  const fetchDeployments = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/apps/${app.id}/deployments`, {
        credentials: "include",
      });
