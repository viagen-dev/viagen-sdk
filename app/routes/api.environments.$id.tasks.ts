...
export async function loader({
  ...
  if (!app) {
    log.warn(
      ...
      "tasks list: app not found or not in org",
    );
  ...
  if (expiredRows.length > 0) {
    ...
    (async () => {
      try {
        ...
        log.info(
          { environmentId, expiredTaskIds, recipientCount: members.length },
          "task timeout emails dispatched",
        );
      } catch (err) {
        log.error({ environmentId, err }, "failed to send task timeout notifications");
      }
    })();
  }
  ...
  log.debug(
    { environmentId, count: filtered.length, statusFilter },
    "tasks listed",
  );
...
export async function action({
  ...
  if (!app) {
    log.warn(
      { userId: user.id, environmentId },
      "task create: app not found or not in org",
    );
    return Response.json({ error: "App not found" }, { status: 404 });
  }
  ...
  log.info(
    { userId: user.id, environmentId, taskId: task.id, taskNumber, branch, model, type },
    "task created",
  );
  return Response.json({
    task: {
