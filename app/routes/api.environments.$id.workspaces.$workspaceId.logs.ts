...
import { environments, workspaces } from "~/lib/db/schema";
...
export async function loader({
  ...
  const [workspace] = await db
    ...
    .from(workspaces)
    .where(
      and(eq(workspaces.id, workspaceId), eq(workspaces.environmentId, id)),
    );
  ...
  log.info(
    { environmentId: id, workspaceId, logsUrl, token: parsed.token },
