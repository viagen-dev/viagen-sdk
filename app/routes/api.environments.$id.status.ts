...
export async function loader({
  ...
  // Single call to resolve all secrets at both app and org level
  const resolved = await resolveAllSecrets(org.id, id);

  const appKeys = new Set(resolved.project.map((s) => s.key));
  const orgKeys = new Set(resolved.org.map((s) => s.key));

  // Helper: check if a key exists and where it comes from (app wins)
  const check = (
    key: string,
  ): { available: boolean; source: "app" | "org" | null } => {
    if (appKeys.has(key)) return { available: true, source: "app" };
    if (orgKeys.has(key)) return { available: true, source: "org" };
    return { available: false, source: null };
  };

  // Helper: get a secret value (app > org)
  ...
  // GitHub
  const githubLinked = !!app.githubRepo;
  const github = check("GITHUB_TOKEN");

  // Vercel
  const vercelLinked = !!app.vercelProjectId;
