...
import type { FeedTask, Environment } from "~/types/task";
...
export function CreateTaskModal({
  ...
  const [appId, setAppId] = useState<string>(defaultEnvironmentId ?? "");
  ...
  // Sync defaultEnvironmentId when it changes externally
  useEffect(() => {
    if (defaultEnvironmentId) setAppId(defaultEnvironmentId);
  }, [defaultEnvironmentId]);
  ...
  useEffect(() => {
    if (open) {
      ...
      setStagedFiles([]);
      setAppId(defaultEnvironmentId ?? "");
      setTimeout(() => titleRef.current?.focus(), 50);
    }
  }, [open, defaultEnvironmentId]);

  const selectedApp = appId
    ? (environments.find((p) => p.id === appId) ?? null)
    : null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/environments/${selectedApp.id}/tasks`, {
        method: "POST",
        credentials: "include",
      ...
      if (!task.environmentName) {
        task.environmentId = selectedApp.id;
        task.environmentName = selectedApp.name;
        task.githubRepo = selectedApp.githubRepo;
        task.vercelProjectId = selectedApp.vercelProjectId;
        task.vercelProjectName = selectedApp.vercelProjectName;
      }
      ...
      for (const file of stagedFiles) {
        ...
        fetch(
          `/api/environments/${selectedApp.id}/tasks/${task.id}/attachments`,
        ...
        <div className="flex flex-col min-h-64">
          ...
          <div className="px-5 pt-2 flex-1">
            <Textarea
              ...
              disabled={!selectedApp || needsClaude || creating}
              rows={3}
