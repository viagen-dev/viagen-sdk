...
export function EnvironmentSettingsPanel({
  ...
  const effectiveGithubRepo = localGithubRepo ?? app.githubRepo;
  const effectiveVercelProjectId =
    localVercelProjectId ?? app.vercelProjectId;
  ...
  // --- App name ---
  const [environmentName, setAppName] = useState(app.name);
  ...
  // --- Secrets ---
  const [appSecrets, setAppSecrets] = useState<SecretEntry[]>([]);
  ...
  const appKeySet = useMemo(
    () => new Set(appSecrets.map((s) => s.key)),
    [appSecrets],
  );
  ...
  const handleSaveName = async () => {
    if (!environmentName.trim() || savingName) return;
    setSavingName(true);
    setNameError(null);
    try {
      const res = await fetch(`/api/environments/${app.id}`, {
  ...
  const fetchConnections = async () => {
    try {
      const res = await fetch(`/api/environments/${app.id}/status`, {
        credentials: "include",
      });
      if (res.ok) {
  ...
  const fetchSecrets = async () => {
    try {
      const res = await fetch(`/api/environments/${app.id}/secrets`, {
        credentials: "include",
      });
  ...
  const fetchSyncState = async () => {
    try {
      const res = await fetch(`/api/environments/${app.id}/vercel-sync`, {
        credentials: "include",
      });
      if (!res.ok) return;
      const data = await res.json();
  ...
  const handleSaveClaude = async () => {
    ...
    try {
      const res = await fetch(`/api/environments/${app.id}/claude`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      toast.success("App Claude key saved");
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>The display name for this app.</CardDescription>
        </CardHeader>
        <CardContent>
          <Input
            type="text"
        ...
        <CardFooter className="border-t justify-between">
          ...
          <Button
            ...
            disabled={!nameChanged || !environmentName.trim() || savingName}
          >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configuration values available to this app.
            {app.vercelProjectId &&
        ...
        <CardContent>
          ...
          {secretsLoading ? (
            ...
            <div className="flex flex-col gap-6">
              ...
              <SecretSection
                title="App"
                badge={<Badge variant="secondary">app</Badge>}
                secrets={appSecrets}
      ...
      {isAdmin && (
        <Card className="border-destructive/50">
          <CardHeader>
            ...
            <CardDescription>
              Permanently delete this app and optionally its linked GitHub
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will permanently delete the app from Viagen.
                ...
                <div className="flex flex-col gap-1.5">
                  <Small>
                    Type{" "}
                    <span className="font-mono font-bold">{app.name}</span>{" "}
                    to confirm
                  </Small>
                  <input
                    type="text"
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && canDelete && handleDeleteApp()
                    }
                    autoComplete="off"
                  />
                </div>
