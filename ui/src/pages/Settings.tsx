...
export function Settings() {
    ...
    apiFetch('/api/integrations/status', currentOrg.id)
      ...
      .then((data) => {
        if (data) {
          ...
          setVercelConnected(data.vercel)
        }
      })
  ...
  const disconnectGithub = async () => {
    await apiFetch('/api/integrations/github', currentOrg.id, { method: 'DELETE' })
    setGithubConnected(false)
  }

  const disconnectVercel = async () => {
    await apiFetch('/api/integrations/vercel', currentOrg.id, { method: 'DELETE' })
      ...
      <div style={styles.section}>
        ...
        <div style={{ ...styles.card, marginBottom: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {loading ? (
              <span style={{ fontSize: '0.8125rem', color: 'var(--ds-gray-400)' }}>...</span>
            ) : githubConnected ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
